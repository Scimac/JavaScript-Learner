- JavaScript (JS) is a versatile scripting language that can be executed in various environments featuring a JS engine. 
- The most common use is within web browsers, where it enables dynamic and interactive content. 
...
- File is executed before the body loads. Good for loading libraries/dependencies.
- Do NOT call any files that uses DOM or Body here, may cause errors.
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
- Good places to call any scripts that are required to load after the body is loaded.
- Example - Animation scripts usually are placed here, as it is imperative for the body to load to execute animation scripts.
